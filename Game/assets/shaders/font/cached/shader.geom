...
layout (set = 1, binding = 0) uniform String {
    uint string[MAX_CHARACTERS];
} string_data;
...
float getStringWidth() {
    ...
    for (int i = 0; i < MAX_CHARACTERS; i++) {
        uint codepoint = string_data.string[i];
...
void main() {
    ...
    if (string_options.center) {
        renderOrigin.x -= getStringWidth() / 2.0f;
    }

    for (int i = 0; i < MAX_CHARACTERS; i++) {
        uint codepoint = string_data.string[i + string_options.string_offset];
        if (codepoint == 0) break;
